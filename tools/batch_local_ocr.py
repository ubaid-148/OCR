...
def code_fingerprint():
    ...
    value.update(json.dumps({k:v for k,v in os.environ.items() if k.startswith(('OCR_', 'PADDLE_'))}, sort_keys=True).encode())
...
def run_batch(pdf_dir, output, extractor=None):
    if extractor is None:
        from coordinate_ocr import extract_pdf
        extractor = extract_pdf
    output = Path(output); output.mkdir(parents=True, exist_ok=True)
    sources = sorted(Path(pdf_dir).glob('*.pdf'))
    if not sources:
        raise ValueError('No PDFs found')
    fingerprint = code_fingerprint()
    rows = []
    for index, source in enumerate(sources, 1):
        signature = {'pdf_sha256': hashlib.sha256(source.read_bytes()).hexdigest(), 'code': fingerprint}
        ...
        if cached and cached.get('signature') == signature and cached.get('complete'):
            ...
            try:
                raw = extractor(source, 'eng+ara', progress=lambda message: print('  '+message, flush=True))
                save(output/(source.stem+'.ocr.json'), raw)
                details = {}
                result = extract_result(raw, source.name, mode='fast', details=details)
...
if __name__ == '__main__':
    ...
    result = run_batch(args.pdf_dir, args.output)
